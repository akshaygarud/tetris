...
public class Graveyard {
	...
	public static void bury(Block[] blocks) {
		//HOMEWORK:
		//it needs to sort the blocks in increasing y order.
		Block temp;
		for(int i=0; i<blocks.length-1; i++) {
			
			for(int  j=0; j<=blocks.length-2; j++) {
	...
	public static void bury(Block block) {
		...
		if (isFullRow(block.getY())) {
			...
			// play the 'eerie' sound.
		}
	}
	
	
	private static boolean isFullRow(int y) {
		for(int x=X_LEFT; x<X_RIGHT-Block.SIZE; x+=Block.SIZE ) {
